...
const ProjectDetail: React.FC = () => {
  ...
  const currentStep = () => {
    switch (delegationState) {
      ...
      case constant.delegationState.CHECKING_SAMPLE.desc: {
        currentStatus = "process";
        tran_pathName = constant.docPath.sample.AUDIT;
        return 19
      }
  ...
  const table_jump = (state: string, stepIndex: number) => {
    switch (stepIndex) {
      ...
      case 19: {
        if (state != constant.delegationState.CHECKING_SAMPLE.desc){
          return "验收样品中";
        } else break;
      }
      case 20: {
      ...
      <Row>
        <ProCard title="流程进度" bordered>
          <Steps className="normalProcess" current={currentStep()} status={currentStatus as any} direction="vertical">
            ...
            <Step stepIndex={19} title="测试部/市场部" description={table_jump(String(delegationState),19)}/>
